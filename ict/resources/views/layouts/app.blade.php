...
<body>
  ...
  <div class="app-wrap">
    ...
    <aside class="sidebar"> 
        <div> 
          <div class="brand"> 
            <img src="{{ asset('images/logo_jakoa.png') }}" class="h-12 w-12 rounded-md shadow-lg" alt="JAKOA"> 
            <div> 
                <div class="text-lg font-bold" style="color:var(--cyan-2)">JAKOA ICT</div> 
                <div class="micro">Sistem Pengurusan Aset</div> 
            </div> 
        </div> 
        ...
        <nav class="mt-3 space-y-2"> 
            ...
            <a href="{{ route('dashboard') }}" class="{{ request()->routeIs('dashboard') ? 'active' : '' }}"> 
                <span class="px-3 py-2 rounded inline-flex items-center gap-3"> 
                    <svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                        <path stroke-width="1.2" d="M3 12l2-2 4 4 8-8 4 4v6a2 2 0 01-2 2H5a2 2 0 01-2-2z"/>
                    </svg> 
                    Dashboard 
                </span> 
            </a> 
            ...
            <div class="sidebar-group"> 
                <button type="button" class="w-full flex items-center justify-between px-3 py-2 rounded {{ request()->routeIs('admin.assets.*') ? 'bg-sky-50 text-sky-700' : 'text-slate-700 hover:bg-slate-50' }}" aria-expanded="{{ request()->routeIs('admin.assets.*') ? 'true' : 'false' }}" onclick="document.getElementById('submenu-admin-assets').classList.toggle('hidden')"> 
                    <span class="inline-flex items-center gap-3"> 
                        <svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                            <path stroke-width="1.2" d="M3 7h18M7 11h10M8 15h8"/>
                        </svg> 
                        Aset Sewaan 
                    </span> 
                    <svg class="h-4 w-4 transform {{ request()->routeIs('admin.assets.*') ? '' : '' }}" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                        <path stroke-width="1.2" d="M6 9l6 6 6-6"/>
                    </svg> 
                </button> 
                <div id="submenu-admin-assets" class="mt-1 ml-6 space-y-1 {{ request()->routeIs('admin.assets.*') ? '' : 'hidden' }}"> 
                    <a href="{{ route('admin.assets.index') }}" class="block px-3 py-2 rounded {{ request()->routeIs('admin.assets.index') ? 'bg-slate-100 font-semibold' : 'hover:bg-slate-50' }}">Manage Assets</a> 
                    <a href="{{ route('admin.assets.create') }}" class="block px-3 py-2 rounded {{ request()->routeIs('admin.assets.create') ? 'bg-slate-100 font-semibold' : 'hover:bg-slate-50' }}">Tambah Aset</a> 
                    <a href="{{ route('admin.assets.export') ?? '#' }}" class="block px-3 py-2 rounded hover:bg-slate-50">Export / Laporan</a> 
                </div> 
            </div> 

            {{-- Pinjaman with submenu --}} 
            <div class="sidebar-group"> 
                <button type="button" class="w-full flex items-center justify-between px-3 py-2 rounded {{ request()->routeIs('admin.pinjaman.*') ? 'bg-sky-50 text-sky-700' : 'text-slate-700 hover:bg-slate-50' }}" aria-expanded="{{ request()->routeIs('admin.pinjaman.*') ? 'true' : 'false' }}" onclick="document.getElementById('submenu-admin-pinjaman').classList.toggle('hidden')"> 
                    <span class="inline-flex items-center gap-3"> 
                        <svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                            <path stroke-width="1.2" d="M12 4v16m8-8H4"/>
                        </svg> 
                        Pinjaman 
                    </span> 
                ...
                <div id="submenu-admin-pinjaman" class="mt-1 ml-6 space-y-1 {{ request()->routeIs('admin.pinjaman.*') ? '' : 'hidden' }}"> 
                    <a href="{{ route('admin.pinjaman.index') }}" class="block px-3 py-2 rounded {{ request()->routeIs('admin.pinjaman.index') ? 'bg-slate-100 font-semibold' : 'hover:bg-slate-50' }}">All Requests</a>
                    <a href="{{ route('admin.pinjaman.approvals') }}" class="block px-3 py-2 rounded {{ request()->routeIs('admin.pinjaman.approvals') ? 'bg-slate-100 font-semibold' : 'hover:bg-slate-50' }}">Approve / Reject</a>
                    <a href="{{ route('admin.pinjaman.history') ?? '#' }}" class="block px-3 py-2 rounded hover:bg-slate-50">Full History</a> 
                </div> 
            </div> 
            
            {{-- Aduan with submenu --}} 
            <div class="sidebar-group"> 
                <button type="button" class="w-full flex items-center justify-between px-3 py-2 rounded {{ request()->routeIs('admin.aduan.*') ? 'bg-sky-50 text-sky-700' : 'text-slate-700 hover:bg-slate-50' }}" aria-expanded="{{ request()->routeIs('admin.aduan.*') ? 'true' : 'false' }}" onclick="document.getElementById('submenu-admin-aduan').classList.toggle('hidden')"> 
                    <span class="inline-flex items-center gap-3"> 
                        <svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                            <path stroke-width="1.2" d="M9 12h6M12 8v8"/>
                        </svg> 
                        Aduan 
                    </span> 
                    <svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                ...
                <div id="submenu-admin-aduan" class="mt-1 ml-6 space-y-1 {{ request()->routeIs('admin.aduan.*') ? '' : 'hidden' }}"> 
                    <a href="{{ route('admin.aduan.list') }}" class="block px-3 py-2 rounded {{ request()->routeIs('admin.aduan.list') ? 'bg-slate-100 font-semibold' : 'hover:bg-slate-50' }}">View Complaints</a> 
                    <a href="{{ route('admin.aduan.status') }}" class="block px-3 py-2 rounded {{ request()->routeIs('admin.aduan.status') ? 'bg-slate-100 font-semibold' : 'hover:bg-slate-50' }}">Update Status / Reply</a> 
                </div> 
            </div> 
            
            {{-- Admin: Permohonan Stok --}} 
            <div class="sidebar-group"> 
                <button type="button" class="w-full flex items-center justify-between px-3 py-2 rounded {{ request()->routeIs('admin.stok.*') ? 'bg-sky-50 text-sky-700' : 'text-slate-700 hover:bg-slate-50' }}" aria-expanded="{{ request()->routeIs('admin.stok.*') ? 'true' : 'false' }}" onclick="document.getElementById('submenu-admin-stok').classList.toggle('hidden')"> 
                    <span class="inline-flex items-center gap-3"> 
                        <svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                ...
                <div id="submenu-admin-stok" class="mt-1 ml-6 space-y-1 {{ request()->routeIs('admin.stok.*') ? '' : 'hidden' }}"> 
                    <a href="{{ route('admin.stok.register') }}" class="block px-3 py-2 rounded {{ request()->routeIs('admin.stok.register') ? 'bg-slate-100 font-semibold' : 'hover:bg-slate-50' }}">Register Stock</a> 
                    <a href="{{ route('admin.stok.requests') }}" class="block px-3 py-2 rounded {{ request()->routeIs('admin.stok.requests') ? 'bg-slate-100 font-semibold' : 'hover:bg-slate-50' }}">View Requests</a> 
                    <a href="{{ route('admin.stok.status') ?? '#' }}" class="block px-3 py-2 rounded hover:bg-slate-50">Update Status</a> 
                </div> 
            </div> 
        </nav> 
    </div>
    ...
    {{-- Admin: Pengurusan Pengguna --}} 
            <div class="sidebar-group"> 
                <button type="button" class="w-full flex items-center justify-between px-3 py-2 rounded {{ request()->routeIs('admin.users.*') ? 'bg-sky-50 text-sky-700' : 'text-slate-700 hover:bg-slate-50' }}" aria-expanded="{{ request()->routeIs('admin.users.*') ? 'true' : 'false' }}" onclick="document.getElementById('submenu-admin-users').classList.toggle('hidden')"> 
                    <span class="inline-flex items-center gap-3"> 
                        <svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                            <path stroke-width="1.2" d="M3 7h18v13H3zM8 3v4"/>
                        </svg> 
                ...
                <div id="submenu-admin-users" class="mt-1 ml-6 space-y-1 {{ request()->routeIs('admin.users.*') ? '' : 'hidden' }}"> 
                    <a href="{{ route('admin.users.index') }}" class="block px-3 py-2 rounded {{ request()->routeIs('admin.users.index') ? 'bg-slate-100 font-semibold' : 'hover:bg-slate-50' }}">User List</a> 
                    <a href="{{ route('admin.users.roles') }}" class="block px-3 py-2 rounded {{ request()->routeIs('admin.users.roles') ? 'bg-slate-100 font-semibold' : 'hover:bg-slate-50' }}">Manage Roles</a> 
                </div> 
            </div> 
        </nav> 
    </div>
